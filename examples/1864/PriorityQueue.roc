...
sink = \store, index, comparator ->
    ...
    left =
        ...
        |> Result.map Value
        |> Result.withDefault LeftMissing
    right =
        List.get store rightIdx
        |> Result.map Value
        |> Result.withDefault RightMissing
        ...
        (_, _) ->
            ...
            |> Result.map \oldValue ->
                if comparator oldValue newValue == GT then
                    ...
                    store
            |> Result.withDefault store

swim = \store, index, currentValue, comparator ->
    if index == 0 then
        store
    else
        parentIndex = (index - 1) |> Num.toFrac |> Num.div 2 |> Num.floor
...
expect
    ...
    values =
        ...
        |> List.walkUntil (queue, []) \(aq, accum), _ ->
            pop aq
